...
// configuration lifecycle events. This is implemented by the KVStoreMesh.
...
func createConfigDirWatcher(path string, lifecycle clusterLifecycle) (*configDirectoryWatcher, error) {
...
func isClusterConfigFile(path string) bool {
	b, err := ioutil.ReadFile(path)
	if err != nil {
		return false
	}

	// search for the "cluster-id:" string
	return strings.Contains(string(b), "cluster-id:")
}
...
func (cdw *configDirectoryWatcher) handleAddedFile(name, absolutePath string) {
	...
	if !isClusterConfigFile(absolutePath) {
		return
	}
...
func (cdw *configDirectoryWatcher) watch() error {
	log.WithField("config", cdw.path).Infof("Starting config directory watcher")
	...
	go func() {
		for {
			...
			case event := <-cdw.watcher.Events:
				...
				log.WithField(fieldRemoteClusterName, name).Debugf("Received fsnotify event: %+v", event)
				switch {
